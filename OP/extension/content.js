...
chrome.storage.onChanged.addListener((changes, namespace) => {
    for (let [key, { oldValue, newValue }] of Object.entries(changes)) {
        if (key === 'syncTime') {
            console.log(`SyncTime changed from ${oldValue} to ${newValue}`);
            const episodeId = chrome.storage.local.get(["episodeId"]);
            const syncTime = chrome.storage.local.get(["syncTime"]);
